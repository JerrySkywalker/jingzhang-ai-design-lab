...
## Direct-near-duplicate test

Each plausibly overlapping package was tested on six axes:
...
## Closest same-territory packages

| Package | Classification | Material distinction from C05 |
| --- | --- | --- |
| `adaptive-jingzhang` | `SAME_TERRITORY_DIFFERENT_PROPOSITION` | Adaptive-network divergence map, strong green ridge and candidate-corridor selection; not a STATUS × ACTION patch field. |
...
reversibility, AI-off fallback, provisional-data disclosure, urban renewal or
the shared three-area brief. These are common taskbook territory, not a
duplicate proposition.

None of the screened peers combines C05's retain-first heterogeneous field,
non-mandatory-spine geometry, `STATUS × ACTION` patch logic, its particular
water/compound—public-campus-threshold—grade-separated-station area roles,
evidence-gated project chain, and task-only AI with ordinary-city continuity.
