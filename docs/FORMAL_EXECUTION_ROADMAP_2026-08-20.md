...
| F01 | Figure build source + receipt | formal `/tools`, `artifact-receipt.json` | C/D/E data | Implementer | F FIGURES | one command produces stable, offline paired outputs and SHA receipt | rebuild hash + asset scan | 08-15 | retain last known source/output; do not edit PNG by hand |
